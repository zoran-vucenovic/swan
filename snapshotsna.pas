unit SnapshotSNA;
// Copyright 2022-2026 Zoran Vučenović
...
type
  ...
  strict private
    function InternalSaveSna(const Stream: TStream; out InvalidSP: Boolean): Boolean;
...
function TSnapshotSNA.InternalSaveSna(const Stream: TStream; out
  InvalidSP: Boolean): Boolean;
var
  Header: THeader;
    ...
    try
      ...
      if Stream.Read(Header{%H-}, SizeOf(Header)) = SizeOf(Header) then begin
        ...
        if State.SpectrumModel <> TSpectrumModel.smNone then begin
          ...
          try
            MS.Size := Int64(RamPagesCount) * KB16;
            P := PByte(MS.Memory);
...
var
  InvalidSP: Boolean;
begin
  Result := InternalSaveSna(Stream, InvalidSP) and (not InvalidSP);

  { This exception can be raised when saving "sna" file format for 48K Spectrum.
  The sna file puts programme counter on the stack. This excepton signals that,
  at the moment of saving, SP - 1 or SP - 2 falls outside of available ram
  memory.
  More info: https://worldofspectrum.org/faq/reference/formats.htm#File, see
  under "SNA Format"
  With 128K this problem is solved, as the programme counter is saved in a
  separate variable in the file and not pushed on the stack. }
  if InvalidSP then
    raise ESnapshotSaveError.Create(
      'SNA file cannot be saved.' + LineEnding + 'Stack pointer points to rom.');
end;
