...
type
  ...
  strict private
    ...
    procedure SetOneKeyPerSpectrumKey(const HalfRowIndex: THalfRowIndex; const WA: Array of Word);
    procedure AddPCKeysToSpectrumKey(const HalfRowIndex: THalfRowIndex; const Index: TKeyIndex; PCKeys: array of Word);
    procedure MapKey(const AKey: Word; const HalfRowIndex: THalfRowIndex; const KeyIndex: TKeyIndex);
    procedure UnmapKey(const AKey: Word; const HalfRowIndex: THalfRowIndex; const KeyIndex: TKeyIndex);
    procedure ClearKeyMaps;
...
{$J+}
const
  DefaultKeysSaved: Boolean = False;
{$pop}
var
  KMRecs: TKeyMapRecs;
begin
  ...
  if not DefaultKeysSaved then begin
    DefaultKeysSaved := True;
    FKeyMapper.SaveToKeyMapRecs(KMRecs);
    TKeyMappings.DefaultMapRecs := KMRecs;
  end;
end;

procedure TSpectrumKeyBoard.SetOneKeyPerSpectrumKey(
...
begin
  if Length(WA) = 5 then begin
    for I := 0 to High(WA) do begin
      ...
      MapKey(WA[I], HalfRowIndex, I);
    end;
  end;
end;

procedure TSpectrumKeyBoard.AddPCKeysToSpectrumKey(
...
begin
  for I := Low(PCKeys) to High(PCKeys) do begin
    MapKey(PCKeys[I], HalfRowIndex, Index);
  end;
end;
...
procedure TSpectrumKeyBoard.ClearKeyMaps;
begin
  FKeyMapper.Clear;
end;

procedure TSpectrumKeyBoard.MapKey(const AKey: Word;
  const HalfRowIndex: THalfRowIndex; const KeyIndex: TKeyIndex);
begin
  FKeyMapper.MapKey(AKey, HalfRowIndex, KeyIndex);
end;
